...
var SS = SpreadsheetApp.openById('1onwduuHF15TcV8zpXG8iaHN8Ykpl3iQvZIX_3t4BQlw');    //Enter Your Sheet ID Got From Sheet URL Link
var sheet = SS.getSheetByName('pzemsheet');      // Enter your sheet name here, In my case it is pzemsheet
var str = "";

function onOpen(){
  var ui = SpreadsheetApp.getUi();
  ui.createMenu('pzem004_v3_logger')
  .addItem('Clear', 'Clear')
  .addToUi();
}
...
function Clear(){
  ...
  SS.toast('Chart cleared', 'pzem004_v3_logger', 5);
}
  ...
  if (parsedData !== undefined){
    ...
    switch (parsedData.command) {
      case "appendRow":
         ...
         var lstrow = tmp.getLastRow() +1;
         
         tmp.appendRow(dataArr);
         tmp.getRange('G'+lstrow).setValue(new Date());
   ...
   tmp.getRange('I1').setValue(lstrow);
...
//  sheet.getRange('B1').setValue(now);
  sheet.getRange('B1').setValue("Voltage");
  sheet.getRange('C1').setValue("Power - W");
  sheet.getRange('D1').setValue("Energy - Wh");
  sheet.getRange('E1').setValue("PF");
  sheet.getRange('F1').setValue("Hz");
